...
namespace StartupProject.Core.Caching
{
    ...
    public partial class MemoryCacheManager : ILocker, IStaticCacheManager
    {
        ...
        public MemoryCacheManager(IEasyCachingProvider provider, IConfiguration configuration)
        {
            _provider = provider;
        ...
        public T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            return _provider.Get(key, acquire, TimeSpan.FromMinutes(cacheTime ?? AVCachingDefaults.CacheTime))
                .Value;
        }
        ...
        public async Task<T> GetAsync<T>(string key, Func<Task<T>> acquire, int? cacheTime = null)
        {
            ...
            var t = await _provider.GetAsync(key, acquire, TimeSpan.FromMinutes(cacheTime ?? AVCachingDefaults.CacheTime)).ConfigureAwait(false);
            return t.Value;
        }
